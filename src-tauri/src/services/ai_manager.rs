...
1. **每个字段必须填写，绝对不允许留空或返回空字符串**。即使原文没有明确表述某个方面，也必须根据全文上下文、学科常识和逻辑关联进行推断分析。
2. 结构化分析字段（researchQuestion 到 selfPositioning）每个字段至少写 **3 句完整的中文句子**，要包含具体细节，不要泛泛而谈。
3. 不要引用原文原句，用自己的学术语言概括。
4. 如果 PDF 文本质量差或不完整，尽最大可能从已有文本中提取，不要放弃任何字段。

## JSON 字段说明（注意：返回 camelCase 字段名）

### 基本信息（从原文直接提取）
- title: 论文标题，字符串。直接从原文提取完整标题，不要修改。
- authors: 作者列表，**必须是 JSON 字符串数组格式**，例如 `["张三", "李四", "Wang Wu"]`。提取所有作者姓名，保留原文格式。注意：多位作者必须分别作为独立的数组元素，不要合并成一个字符串。
- year: 发表年份，整数。四位数年份，从期刊信息、页眉或引用格式中提取。
- abstract: 摘要，字符串。如原文有 Abstract 部分则完整提取；如没有，根据引言和结论自行撰写 4-6 句概括。
...
const FILL_EMPTY_PROMPT: &str = r#"以下是一篇学术论文的结构化分析结果，其中有些字段为空字符串。你的任务是根据已有字段的内容和学术常识，推断并补充所有空字段。
...
1. 每个空字段必须填写至少 3 句具体推断内容
2. 不要写"根据已有信息无法推断"——必须给出合理的学术推断
3. 推断内容必须符合学术规范，不要编造不存在的细节
4. 输出完整的 JSON 对象，包含所有字段，已有的保持不变，空的用推断内容填充
...
const RELATION_PROMPT: &str = r#"分析以下论文之间的学术论争关系。
...
2. 方法论层面：A 和 B 的方法是否互补、继承或对立？
3. 理论层面：A 和 B 的理论框架是否相同、冲突或递进？
4. 结论层面：A 和 B 的实证发现是否一致、矛盾或互补？
5. 自述定位：是否明确提到对方？是支持、反对还是限定？
6. 分别从 A→B 和 B→A 两个方向判断，每个方向可能有不同关系
...
4. confidence 标准：0.9+=明确定义关系，0.7+=较强证据，0.5+=合理推断，低于 0.5 不输出
5. 最多输出 20 条

## 输出格式
只返回 JSON 数组，不要任何解释文字：
[{"sourceId":"A的id","targetId":"B的id","type":"关系类型","confidence":0.85,"evidence":"A主张...，B主张...，两者..."}]
...
4. confidence 标准：0.9+=明确定义，0.7+=较强证据，0.5+=合理推断，低于 0.5 不输出
5. 最多输出 20 条

## 输出格式
只返回 JSON 数组：
[{"sourceId":"A的id","targetId":"B的id","type":"关系类型","confidence":0.85,"evidence":"具体对比分析"}]
