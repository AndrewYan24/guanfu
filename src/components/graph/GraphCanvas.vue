...
function exportPng() {
  if (!cy.value) return;
  const pngDataUrl = cy.value.png({ full: true, scale: 2, bg: '#FFFFFF' });
  ...
  img.onload = () => {
    ...
    if (!ctx) { downloadImage(pngDataUrl); return; }

    ctx.drawImage(img, 0, 0);
    const text = '由观复生成';
    const fontSize = Math.max(14, Math.round(img.width * 0.012));
    ctx.font = `${fontSize}px "HarmonyOS Sans", -apple-system, sans-serif`;
    ctx.fillStyle = 'rgba(0, 0, 0, 0.25)';
    const metrics = ctx.measureText(text);
    const padding = Math.round(fontSize * 0.8);
    ctx.fillText(text, img.width - metrics.width - padding, img.height - padding);
    downloadImage(canvas.toDataURL('image/png'));
  };
  img.onerror = () => downloadImage(pngDataUrl);
  img.src = pngDataUrl;
}
