...
function exportPng() {
  ...
  graphImg.onload = () => {
    ...
    if (settingsStore.watermarkEnabled) {
      ...
      const text = locale.value === 'zh-tra'
        ? '由觀復產生'
        : locale.value === 'zh-sim'
          ? '由观复生成'
          : 'Generated by Guanfu';
...
async function triggerDownload(dataUrl: string) {
  const appName = locale.value === 'zh-tra' ? '觀復' : 'Guanfu';
  const projectName = projectStore.currentProject?.name || 'graph';
  const filePath = await save({
    defaultPath: `${appName}-${projectName}.png`,
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
  ...
  const projectName = projectStore.currentProject?.name || 'graph';
  const filePath = await save({
    defaultPath: `${appName}-${projectName}.svg`,
